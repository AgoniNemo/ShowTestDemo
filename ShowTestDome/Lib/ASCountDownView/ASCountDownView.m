...
-(void)creatUI
{
    self.showType = ShowTimeTypeColonHourMinuteSec;
...
-(void)startCountDown
{
    ...
    NSDate *startDate= [NSDate dateWithTimeIntervalSince1970:[self.endTime doubleValue]];
...
-(void)setActivityColor:(UIColor *)activityColor
{
    ...
    self.backgroundColor = activityColor;
}
-(void)destoryTimer{
    if (_timer) {
    ...
    self.timeLabel.text = (_type == TimeTypeCountdownEnd)? @"活动结束" : @"立即拍";
    
    if (!self.endColor) {
        self.endColor = [UIColor colorWithRed:155.0/255.0 green:155.0/255.0 blue:155.0/255.0 alpha:1];
    }
    self.backgroundColor = (_type == TimeTypeCountdownEnd)? self.endColor : self.activityColor;
